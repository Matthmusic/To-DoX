...
export function normalizeUsers(value: unknown): User[] {
  ...
  for (const user of value) {
    ...
    byId.set(user.id, {
      ...
      // Absent pour les users pas encore migrés vers le backend (voir types.ts) — ne pas
      // le fabriquer ici, juste le transmettre tel quel quand l'API le fournit.
      ...(user.role === 'admin' || user.role === 'member' ? { role: user.role } : {}),
    });
  }
