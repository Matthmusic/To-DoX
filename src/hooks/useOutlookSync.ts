 ...
 *  - Écriture : régénère todox-tasks.ics avec debounce 2s après modif des tâches
 *
...
export function useOutlookSync() {
    ...
    // ─── Export ICS sortant (To-DoX → Outlook) ────────────────────────────────
    useEffect(() => {
        ...
        exportTimer.current = setTimeout(async () => {
            try {
                ...
                if (writeRes.success) {
                    devLog('💾 [OUTLOOK] todox-tasks.ics exporté:', pathRes.path);
    ...
    return { fetchOutlookEvents, icsExportPath, icsServerUrl };
}
