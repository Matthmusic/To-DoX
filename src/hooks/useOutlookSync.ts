 ...
 *
 * L'export ICS local (todox-tasks.ics / todox-tasks-view.ics) nécessitait un dossier
 * de stockage local (OneDrive) ; l'app étant désormais connectée au serveur de l'équipe,
 * il n'y a plus d'emplacement local où écrire ces fichiers — l'export est désactivé
 * (icsExportPath/icsViewPath/icsServerUrl restent toujours null).
 *
...
export function useOutlookSync() {
    ...
    const [icsExportPath] = useState<string | null>(null);
    const [icsViewPath] = useState<string | null>(null);
